...
function LatestInfractions({ t, projectsInfractions, filterNews, getTags }) {
  useEffect(() => {
    filterNews();
    getTags();
  }, []);
  ...
  const infractionData = projectsInfractions.map((n) => ({
    infraction: (
      <div>
  ...
  return (
    <Box
      ...
      header={
        <div>
          <h4 className={"text-center text-lg"}>{t("TEXT_LATEST_MONITORING_INFRACTIONS")}</h4>
        </div>
      }
      footer={
        <Link to="/monitoring-infractions" className={"small-box-footer"}>
...
LatestInfractions.propTypes = {
  projectsInfractions: PropTypes.arrayOf(PropTypes.object),
};
...
function mapStateToProps(state) {
  return {
    projectsInfractions: state.data.projectsInfractions,
  };
}
